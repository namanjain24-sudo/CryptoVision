  ...
  useEffect(() => {
    ...
    const fetchCryptoData = async () => {
      try {
        ...
        // Ensure loading state is always updated regardless of mount status
        setLoading(false);
        // Checks in try/catch already prevent updates if unmounted before finally
      }
    };
  ...
  return (
    <div className="min-h-screen bg-black text-white py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Results */}
        {error ? (
          <div className="text-center py-12 text-red-400  rounded-2xl  ">
            {/* <p>{error}</p> */}
            <BeatLoader color="#07a825" loading margin={5} size={20} />
          </div>
        ) : loading ? (
